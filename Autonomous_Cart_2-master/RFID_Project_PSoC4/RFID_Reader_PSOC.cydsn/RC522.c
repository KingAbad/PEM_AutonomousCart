...
void PCD_WriteRegister(unsigned char addr, unsigned char value) {
         SPI_ClearFIFO(); SPI_ClearRxBuffer();//SS = false;
...
unsigned char PCD_ReadRegister(unsigned char addr) {
   ...
   SPI_WriteTxData(((0x00<<1)&0x7E)|0x00);  
   CyDelayUs(500);  SPI_ReadRxData(); 
...
void PCD_ClearRegisterBitMask(	unsigned char reg,	///< The register to update. One of the PCD_Register enums.
	...
	unsigned char tmp;
	tmp = PCD_ReadRegister(reg);
PCD_WriteRegister(reg,(~ mask & tmp));		//PCD_WriteRegister(reg,(tmp&(~mask)));		// clear bit mask
...
void PCD_Init(void) {
	...
	PCD_WriteRegister(TModeReg, 0x80);			// TAuto=1;   таймер запускается автоматически в конце передачи во всех режимах связи на всех скоростях     timer starts automatically at the end of the transmission in all communication modes at all speeds
	PCD_WriteRegister(TPrescalerReg, 0xA9);		// TPreScaler = TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 => f_timer=40kHz, ie a timer period of 25?s.
	PCD_WriteRegister(TReloadRegH, 0x03);		// Reload timer with 0x3E8 = 1000, ie 25ms before timeout.
	PCD_WriteRegister(TReloadRegL, 0xE8);
	PCD_WriteRegister(TxASKReg, 0x40);		// Default 0x00. Принудительная модуляция 100% ASK независимо от настройки регистра ModGsPReg   Force a 100 % ASK modulation independent of the ModGsPReg register setting
	PCD_WriteRegister(ModeReg, 0x3D);		// Default 0x3F. Установите предустановленное значение для сопроцессора CRC для CalcCRC   Set the preset value for the CRC coprocessor for the CalcCRC command to 0x6363 (ISO 14443-3 part 6.2.4)
